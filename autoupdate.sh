...
ALLOWED_EXTRA_PATTERNS="${ALLOWED_EXTRA_PATTERNS:-site=linux.abitti.fi}"
...
if [[ -n "${ALLOWED_EXTRA_ORIGINS:-}" ]]; then
    ...
    if [[ -n "${ALLOWED_EXTRA_PATTERNS:-}" ]]; then
        patterns_raw="$(printf '%s\n' "${ALLOWED_EXTRA_PATTERNS}" | tr ' ' '\n' | sed '/^[[:space:]]*$/d')"
        while IFS= read -r pat; do
            [[ -z "$pat" ]] && continue
            extra_patterns_lines+="    \"${pat}\";\n"
        done <<< "${patterns_raw}"
        if [[ -n "$extra_patterns_lines" ]]; then
            log "Adding extra Origins-Pattern entries:"
            printf '%b' "$extra_patterns_lines" | while IFS= read -r l; do
                [[ -n "$l" ]] && log "  $l"
            done
        fi
    fi
